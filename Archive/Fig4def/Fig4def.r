...
niche_overlap_16S <- niche.overlap(t_ASV_filtered_16S, method = "schoener")%>% 
  data.matrix() %>% 
...
niche_overlap_16S_Boxplot_tmp$Group<-c("Prokaryotes")
...
niche_overlap_ITS <- niche.overlap(t_ASV_filtered_ITS, method = "schoener")%>% 
  data.matrix() %>% 
...
niche_overlap_ITS_Boxplot_tmp$Group<-c("Eukaryotes")
...
########## calculate 16S Ocom for each group #######
...
for (df_name in data_frames) {
  ...
  niche_overlap <- niche.overlap(get(df_var), method = "schoener") %>%
    data.matrix() %>%
...
########## calculate ITS Ocom for each group #######
...
niche_overlap_All_Boxplot$Group <- factor(niche_overlap_All_Boxplot$Group,levels=c('Prokaryotes', 'Eukaryotes'))
...
#Calculate Calculate CK_Black_Green_White_16S_Ocom
...
#Calculate Calculate CK_Black_Green_White_ITS_Ocom
...
ggsave("Ocom_all_3*11.pdf", plot = p1, width = 11, height = 3, units = "in")
