...
p1<-ggplot(Survival_GO_abundance_plot,aes(GO_Term,value,color=Functions, fill=Functions))+
  ...
  geom_jitter(shape=16,alpha=0.8,size=2.5,width = 0.27, show.legend=T)+
  ...
  xlab(NULL)+
  geom_vline(aes(xintercept =2.5),linetype="dashed", size=0.6, colour="lightgray")+
  geom_vline(aes(xintercept =7.5),linetype="dashed", size=0.6, colour="lightgray")+
