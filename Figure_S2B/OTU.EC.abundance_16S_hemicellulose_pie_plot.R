...
OTU_EC_pie<-OTU_EC[,which(colnames(OTU_EC)%in%c("EC.3.2.1.8","EC.3.2.1.32","EC.3.2.1.37",
...
EC321136_plot<-EC_pie_plot[,c(1,6)]

EC321136_plot%>% group_by(Phylum) %>% summarise_each(funs(sum))->EC321136_plot_summary

EC321156_plot<-EC_pie_plot[,c(2,6)]

EC321156_plot%>% group_by(Phylum) %>% summarise_each(funs(sum))->EC321156_plot_summary

EC321177_plot<-EC_pie_plot[,c(3,6)]

EC321177_plot%>% group_by(Phylum) %>% summarise_each(funs(sum))->EC321177_plot_summary

EC32137_plot<-EC_pie_plot[,c(4,6)]

EC32137_plot%>% group_by(Phylum) %>% summarise_each(funs(sum))->EC32137_plot_summary

EC3218_plot<-EC_pie_plot[,c(5,6)]

EC3218_plot%>% group_by(Phylum) %>% summarise_each(funs(sum))->EC3218_plot_summary



plot_all<-cbind(EC321136_plot_summary,EC321156_plot_summary,
                EC321177_plot_summary,EC32137_plot_summary,EC3218_plot_summary)
...
mycolor<-c(rev(brewer.pal(11,"RdYlGn"))[2:4],rev(brewer.pal(8,"Accent")))
