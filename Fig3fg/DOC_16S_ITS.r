...
library(DOC) # install.packages("DOC", repos = NULL, type = "source")
...
p2<-plot(DOC_ITS_results) + 
  theme_bw() +
  geom_text(aes(x = 0.25,y = 0.95,
                label = paste("Fungi", '\n', "Fns=",round(Fns_ITS_value,4), '\n', "P<", round(p_ITS_value, 4)))) +#调节x和y调节字体位置
  theme(axis.text.x = element_text(colour = 'blue'),
        axis.title.x = element_text(colour = 'blue'),
        axis.text.y = element_text(colour='red'),
        axis.title.y = element_text(colour = 'red'))
