...
# ASV_filtered_ITS_Tax <- ASV_filtered_ITS_Tax %>%
...
ps_ITS<-phyloseq(otu_table(as.matrix(ASV_filtered_ITS),taxa_are_rows = TRUE),
                      ...
                      sample_data(infor)) # 这里非得是data.frame不可。。。
...
p2 <- ggplot(Robustness_dat1, aes(Number.hub.removed, remain.mean, color = Group)) +
  ...
  geom_errorbar(aes(ymin = remain.mean - remain.sd, ymax = remain.mean + remain.sd), width = 0.2, alpha=0.6) + 
...
p3 <- ggplot(Robustness_dat2, aes(Proportion.removed, remain.mean, color = Group)) +
  ...
  geom_errorbar(aes(ymin = remain.mean - remain.sd, ymax = remain.mean + remain.sd), width = 0.02, alpha=0.6) +  
